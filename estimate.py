...
class HeuristicCpp:
	'''Same as class Heuristic, but uses a C++ library implementation under the hood.'''

	def __init__(self, overmat, pref, lobs):
		'''Initialize the heuristic with the global context.'''
		lestimate = ctypes.cdll.LoadLibrary("./estimate.so")
		self.lestimate = lestimate
		self.context = self.__make_context(lestimate, overmat, pref, lobs)

	def cleanup(self):	
		self.lestimate.destroy_context(self.context)
		del self.context
		del self.lestimate
	...
	def __call__(self, lefts, A, Z):
		'''Run one estimation search for the given characteristics of the partial solution.'''
		estimate = self.lestimate.estimate # estimate function
		...
		result = estimate(l, c_lefts, A, Z, self.context)
		return result
	
	def __make_context(self, lestimate, overmat, pref, lobs):
		create_context = lestimate.create_context # create_context function
		create_context.restype = ctypes.c_void_p

		n = len(lobs)                 # length of obs list, overmat etc.
		p = max(map(len,pref))        # length of one pref list
		...
		# print("lestimate context = {0}".format(hex(context)))
		return context
